/* search-provider.c generated by valac 0.26.1, the Vala compiler
...
static gboolean search_provider_can_parse (gchar** terms, int terms_length1) {
	...
	GError * _inner_error_ = NULL;
	{
		...
		_tmp4_ = _tmp3_;
		_tmp5_ = g_strconcat ("gnome-calculator --solve ", _tmp4_, NULL);
		_tmp6_ = _tmp5_;
		g_spawn_command_line_sync (_tmp6_, NULL, NULL, &_tmp7_, &_inner_error_);
		exit_status = _tmp7_;
		_g_free0 (_tmp6_);
		_g_free0 (_tmp4_);
		_g_free0 (_tmp2_);
		...
		_tmp8_ = exit_status;
		g_spawn_check_exit_status (_tmp8_, &_inner_error_);
		if (G_UNLIKELY (_inner_error_ != NULL)) {
			if (_inner_error_->domain == G_SPAWN_ERROR) {
				goto __catch0_g_spawn_error;
			}
	...
	__catch0_g_spawn_error:
	{
		...
		_inner_error_ = NULL;
		_tmp9_ = e->message;
		g_error ("search-provider.vala:32: Failed to spawn Calculator: %s", _tmp9_);
		_g_error_free0 (e);
	}
	goto __finally0;
	__catch0_g_error:
...
GHashTable** search_provider_get_result_metas (SearchProvider* self, gchar** results, int results_length1, int* result_length1) {
	...
	g_return_val_if_fail (_tmp0__length1 == 1, NULL);
	...
	_vala_array_add2 (&argv, &argv_length1, &_argv_size_, NULL);
	{
		...
		_tmp11_ = g_subprocess_newv (_tmp10_, G_SUBPROCESS_FLAGS_STDOUT_PIPE, &_inner_error_);
		_tmp9_ = _tmp11_;
		if (G_UNLIKELY (_inner_error_ != NULL)) {
	...
	__catch1_g_error:
	{
		...
		_inner_error_ = NULL;
		_tmp13_ = e->message;
		g_error ("search-provider.vala:79: Failed to spawn Calculator: %s", _tmp13_);
		_g_error_free0 (e);
	}
	__finally1:
	if (G_UNLIKELY (_inner_error_ != NULL)) {
	...
	{
		gchar* _tmp14_ = NULL;
		g_subprocess_communicate_utf8 (subprocess, NULL, NULL, &_tmp14_, NULL, &_inner_error_);
		_g_free0 (stdout_buf);
		stdout_buf = _tmp14_;
		if (G_UNLIKELY (_inner_error_ != NULL)) {
	...
	__catch2_g_error:
	{
		...
		_inner_error_ = NULL;
		_tmp15_ = e->message;
		g_error ("search-provider.vala:88: Failed reading result: %s", _tmp15_);
		_g_error_free0 (e);
	}
	__finally2:
	if (G_UNLIKELY (_inner_error_ != NULL)) {
	...
	_tmp16_ = g_new0 (GHashTable*, 1 + 1);
	metadata = _tmp16_;
	metadata_length1 = 1;
	_metadata_size_ = metadata_length1;
	_tmp17_ = g_str_hash;
	_tmp18_ = g_str_equal;
	_tmp19_ = g_hash_table_new_full (_tmp17_, _tmp18_, _g_free0_, _g_variant_unref0_);
	...
	_tmp22_ = g_strdup ("id");
	_tmp23_ = results;
	_tmp23__length1 = results_length1;
	_tmp24_ = _tmp23_[0];
	_tmp25_ = _variant_new1 (_tmp24_);
	...
	_tmp31_ = _tmp30_;
	_tmp32_ = g_strconcat (_tmp31_, stdout_buf, NULL);
	_tmp33_ = _tmp32_;
	_tmp34_ = _variant_new2 (_tmp33_);
	g_hash_table_insert (_tmp26_, _tmp27_, _tmp34_);
	_g_free0 (_tmp33_);
	_g_free0 (_tmp31_);
	_tmp35_ = metadata;
...
static void search_provider_spawn_and_display_equation (gchar** terms, int terms_length1) {
	...
	__catch3_g_spawn_error:
	{
		...
		_inner_error_ = NULL;
		_tmp7_ = e->message;
		g_error ("search-provider.vala:108: Failed to spawn Calculator: %s", _tmp7_);
		_g_error_free0 (e);
	}
	__finally3:
	if (G_UNLIKELY (_inner_error_ != NULL)) {
