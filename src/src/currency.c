/* currency.c generated by valac 0.26.1, the Vala compiler
...
static Currency* currency_manager_add_currency (CurrencyManager* self, const gchar* short_name) {
	...
	_tmp8_ = short_name;
	g_warning ("currency.vala:132: Currency %s is not in the currency table", _tmp8_);
	_tmp9_ = short_name;
	_tmp10_ = short_name;
	_tmp11_ = short_name;
...
static void currency_manager_load_imf_rates (CurrencyManager* self) {
	...
	__catch0_g_error:
	{
		...
		g_warning ("currency.vala:222: Failed to read exchange rates: %s", _tmp160_);
		_g_error_free0 (e);
		_g_free0 (data);
		_g_free0 (filename);
					...
					if (_tmp185_ < _tmp186__length1) {
						...
						if (_tmp192_ != NULL) {
							...
							if (_tmp195_ == NULL) {
								...
								_tmp199_ = symbol;
								g_debug ("currency.vala:266: Using IMF rate of %s for %s", _tmp198_, _tmp199_);
								_tmp200_ = symbol;
							...
							_tmp211_ = _tmp210_[0];
							g_warning ("currency.vala:274: Unknown currency '%s'", _tmp211_);
						}
						_g_free0 (symbol);
					}
				}
...
static void currency_manager_set_ecb_rate (CurrencyManager* self, xmlNode* node, Currency* eur_rate) {
	...
	if (_tmp15_) {
		...
		_tmp22_ = value;
		_tmp23_ = name;
		g_debug ("currency.vala:296: Using ECB rate of %s for %s", _tmp22_, _tmp23_);
		_tmp24_ = name;
...
static void currency_manager_set_ecb_fixed_rate (CurrencyManager* self, const gchar* name, const gchar* value, Currency* eur_rate) {
	...
	_tmp0_ = value;
	_tmp1_ = name;
	g_debug ("currency.vala:307: Using ECB fixed rate of %s for %s", _tmp0_, _tmp1_);
	_tmp2_ = name;
...
static void currency_manager_load_ecb_rates (CurrencyManager* self) {
	...
	_tmp1_ = eur_rate;
	if (_tmp1_ == NULL) {
		g_warning ("currency.vala:321: Cannot use ECB rates as don't have EUR rate");
	...
	if (_tmp7_ == NULL) {
		const gchar* _tmp8_ = NULL;
		_tmp8_ = filename;
		g_warning ("currency.vala:334: Couldn't parse ECB rate file %s", _tmp8_);
		_g_free0 (filename);
		_g_object_unref0 (eur_rate);
		return;
	}
	...
	_tmp11_ = xpath_ctx;
	if (_tmp11_ == NULL) {
		g_warning ("currency.vala:341: Couldn't create XPath context");
	...
	_tmp15_ = xpath_obj;
	if (_tmp15_ == NULL) {
		g_warning ("currency.vala:349: Couldn't create XPath object");
		_xmlXPathFreeContext0 (xpath_ctx);
		_g_free0 (filename);
		_g_object_unref0 (eur_rate);
		return;
...
static gboolean currency_manager_load_rates (CurrencyManager* self) {
		...
		for (c_it = c_collection; c_it != NULL; c_it = c_it->next) {
			...
			c = _tmp5_;
			{
				...
				if (_tmp6_) {
					...
					_tmp17_ = _tmp16_;
					_tmp18_ = _tmp17_;
					g_warning ("currency.vala:385: Currency %s is not provided by IMF or ECB", _tmp18_);
					_g_free0 (_tmp18_);
				}
				_g_object_unref0 (c);
			}
	...
	g_debug ("currency.vala:387: Rates loaded");
	loaded_rates = TRUE;
...
Number* currency_manager_get_value (CurrencyManager* self, const gchar* currency) {
	...
	if (_tmp1_) {
		...
		g_debug ("currency.vala:402: Downloading rates from the IMF...");
		_tmp5_ = path;
	...
	if (_tmp7_) {
		...
		g_debug ("currency.vala:409: Downloading rates from the ECB...");
		_tmp11_ = path;
...
static void currency_manager_download_imf_cb (CurrencyManager* self, GObject* object, GAsyncResult* _result_) {
	...
	f = _tmp1_;
	{
		...
		g_debug ("currency.vala:441: IMF rates updated");
	}
	goto __finally1;
	__catch1_g_error:
	{
		...
		g_warning ("currency.vala:445: Couldn't download IMF currency rate file: %s", _tmp4_);
		_g_error_free0 (e);
	}
	__finally1:
	if (G_UNLIKELY (_inner_error_ != NULL)) {
...
static void currency_manager_download_ecb_cb (CurrencyManager* self, GObject* object, GAsyncResult* _result_) {
	...
	f = _tmp1_;
	{
		...
		g_debug ("currency.vala:458: ECB rates updated");
	}
	goto __finally2;
	__catch2_g_error:
	{
		...
		_tmp3_ = e;
		_tmp4_ = _tmp3_->message;
		g_warning ("currency.vala:462: Couldn't download ECB currency rate file: %s", _tmp4_);
		_g_error_free0 (e);
	}
	__finally2:
	if (G_UNLIKELY (_inner_error_ != NULL)) {
